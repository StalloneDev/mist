...
function NumCell({ value }: { value: any }) {
    ...
    return <span className="font-bold text-foreground">{n.toLocaleString("fr-FR")}</span>;
}
...
function DetailModal({ row: initialRow, onClose }: { row: any; onClose: () => void }) {
    const [row, setRow] = useState<any>(initialRow);
    const [isLoading, setIsLoading] = useState(true);

    useEffect(() => {
        if (!initialRow || !initialRow.id) return;
        setIsLoading(true);
        getReportingRowById(initialRow.id).then(res => {
    ...
    if (isLoading) return (
        <div className="reporting-modal-wrapper">
            <div className="reporting-modal-overlay" onClick={onClose} />
            <div className="reporting-modal-container flex flex-col items-center justify-center min-h-[400px]">
                <svg className="animate-spin text-primary w-12 h-12 mb-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 12a9 9 0 1 1-6.219-8.56" /></svg>
                <p className="text-muted font-bold text-xs uppercase tracking-widest">Récupération des détails...</p>
            </div>
        </div>
    );
    ...
    const groups = [
        {
            ...
            fields: [
                { label: "Date Visite", value: formatDate(String(row.date_visite)) },
    ...
    return (
        <div className="reporting-modal-wrapper">
            ...
            {/* Container */}
            <div className="reporting-modal-container transition-all scale-100 animate-in fade-in zoom-in duration-300">


                {/* Modal Header */}
                <div className="reporting-modal-header">
                    <div className="flex items-center gap-6">
                        <div className="w-8 h-8 rounded-3xl bg-primary border-2 border-white/10 flex items-center justify-center text-foreground shadow-2xl shadow-primary/40">
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-4 mt-2">
                                ...
                                <span className="text-[11px] font-bold text-muted uppercase tracking-widest italic">Enregistrement fait Par :  {row.commercial || 'Système'}</span>
                            </div>
                        </div>
                    </div>
                </div>

                <div className="flex-1 overflow-y-auto p-8 custom-scrollbar reporting-modal-body" style={{ maxHeight: "calc(90vh - 200px)" }}>
                                ...
                                <div className="space-y-3 px-3">
                                    {group.fields.map((f, fidx) => (
                                        <div key={fidx} className="flex flex-col gap-0.5 group/field">
                                            ...
                                            <div className="text-sm font-semibold text-foreground group-hover/field:text-primary transition-colors">
                                                {f.value !== null && f.value !== "" ? String(f.value) : <span className="text-slate-300 italic text-xs">Non spécifié</span>}
                                            </div>
                                        </div>
                                    ))}
                                </div>
                    ...
                    {row.observations && (
                        <div className="mt-12 space-y-4 pt-10 border-t border-white/5">
                            ...
                            <div className="p-6 rounded-2xl bg-white/[0.02] border border-white/5 text-sm text-muted leading-relaxed font-medium">
                ...
                <div className="reporting-modal-footer">
                    <button onClick={onClose} className="group flex items-center gap-3 px-10 py-4 rounded-2xl bg-emerald-500 hover:bg-emerald-400 text-foreground text-[10px] font-black uppercase tracking-widest shadow-xl shadow-emerald-500/20 transition-all active:scale-95">
...
export default function ReportingPage() {
    ...
    const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            if (dbRes.success && dbRes.data) {
                ...
                toast.success("Importation Excel réussie");
            }
        } catch (err: any) {
    ...
    return (
        <main className="p-4 sm:p-6 lg:p-8 max-w-screen-2xl mx-auto space-y-6 sm:space-y-8">
            {/* Header */}
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 lg:gap-8 mb-4">
                <div className="space-y-2 sm:space-y-3">
                    ...
                    <h1 className="text-2xl sm:text-3xl lg:text-5xl font-black text-foreground tracking-tight uppercase">Reporting <span className="text-primary prose-glow">Fiches Terrain</span></h1>
                    <p className="text-muted font-medium max-w-2xl text-sm lg:text-lg leading-relaxed hidden sm:block">
            ...
            {/* Ultra-Compact Filters - Single Line / Multi-column on mobile */}
            <div className="premium-panel px-4 py-3 flex flex-wrap lg:flex-nowrap items-center gap-3 shadow-2xl relative overflow-hidden group border-white/5">
                ...
                <div className="flex-1 relative">
                    <input
                        type="text"
                        className="w-full bg-white/[0.03] border border-white/10 focus:border-primary/30 focus:bg-white/[0.06] transition-all py-2.5 pl-10 pr-4 rounded-lg text-sm placeholder:text-muted outline-none"
                ...
                {/* Sector Filter */}
                <div className="flex-1 min-w-[120px] lg:w-[140px] lg:flex-none">
                    <select className="w-full bg-white/[0.03] border border-white/10 py-2.5 px-3 rounded-lg text-xs appearance-none cursor-pointer text-foreground outline-none focus:border-primary/30" value={filterSecteur} onChange={e => handleSecteur(e.target.value)} disabled={initialLoading || rows.length === 0}>
                        <option value="" className="bg-surface-1">Secteur: Tout</option>
                        {secteurs.map(s => <option key={s} value={s} className="bg-surface-1">{s}</option>)}
                    </select>
                </div>


                {/* Potential Filter */}
                <div className="flex-1 min-w-[120px] lg:w-[130px] lg:flex-none">
                    <select className="w-full bg-white/[0.03] border border-white/10 py-2.5 px-3 rounded-lg text-xs appearance-none cursor-pointer text-foreground outline-none focus:border-primary/30" value={filterOpportunite} onChange={e => handleOpp(e.target.value)} disabled={initialLoading || rows.length === 0}>
                        <option value="" className="bg-surface-1">Potentiel: Tout</option>
                        <option value="Forte" className="bg-surface-1">Fort</option>
                        <option value="Moyenne" className="bg-surface-1">Moyen</option>
                        <option value="Faible" className="bg-surface-1">Faible</option>
                    </select>
                </div>


                {/* Commercial Filter */}
                <div className="flex-1 min-w-[120px] lg:w-[160px] lg:flex-none">
                    <select className="w-full bg-white/[0.03] border border-white/10 py-2.5 px-3 rounded-lg text-xs appearance-none cursor-pointer text-foreground outline-none focus:border-primary/30" value={filterCommercial} onChange={e => handleCom(e.target.value)} disabled={initialLoading || rows.length === 0}>
                        <option value="" className="bg-surface-1">Commercial: Tous</option>
                        {commerciaux.map(c => <option key={c} value={c} className="bg-surface-1">{c}</option>)}
                    </select>
                </div>
            ...
            <div className={`premium-panel overflow-hidden border border-white/5 shadow-2xl ${!initialLoading && rows.length === 0 && !loading ? 'pb-8 pt-4' : ''}`}>
                <div className="px-8 py-5 flex items-center justify-between border-b border-white/5 bg-white/[0.01]">
                    ...
                    <div className="flex items-center gap-6">
                        <div className="hidden md:flex items-center gap-2 px-3 py-1 rounded-lg bg-white/5 border border-white/10">
                            <span className="w-3 h-3 text-muted"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M5 12h14" /><path d="m13 18 6-6-6-6" /></svg></span>
                ...
                {initialLoading ? (
                    <div className="py-24 text-center">
                        ...
                        <h3 className="text-muted font-bold tracking-widest uppercase text-xs">Chargement depuis la base...</h3>
                    </div>
                    ...
                    <div className="text-center py-16">
                        ...
                        <h2 className="text-lg font-black text-foreground mb-2">Base de données d&apos;intelligence vide</h2>
                        <p className="text-muted font-medium max-w-sm mx-auto text-sm">
                        ...
                        <table className="w-full text-left text-sm">
                            <thead>
                                <tr className="bg-surface-2 border-b-2 border-primary/20">
                                    {COLUMNS.map(col => (
                                        <th key={col.key} className="px-6 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-muted whitespace-nowrap sticky top-0 bg-surface-2/80 backdrop-blur-md border-r border-border last:border-r-0" style={{ width: col.width, minWidth: col.width }}>
                                ...
                                {paginatedRows.map((row, idx) => (
                                    <tr key={idx} className="hover:bg-white/[0.02] transition-colors group">
                                        {COLUMNS.map(col => {
                                            ...
                                            // Colonne Date Visite
                                            if (col.key === "date_visite") return (
                                                <td key={col.key} className="px-6 py-4 text-muted whitespace-nowrap border-r border-border last:border-r-0" style={{ minWidth: col.width }}>
                                                    <div className="truncate font-bold text-foreground" style={{ minWidth: col.width }}>
                                                        {val ? formatDate(String(val)) : <span className="text-slate-300 italic text-xs">Non renseigné</span>}
                                                    </div>
                                                </td>
                                            );
                                            if (col.key === "raison_sociale") return (
                                                <td key={col.key} className="px-5 py-4 whitespace-nowrap font-bold text-foreground group-hover:text-primary transition-colors" style={{ minWidth: col.width }}>
                                                ...
                                                <td key={col.key} className="px-5 py-4 whitespace-nowrap">
                                                    <div className="flex items-center gap-2">
                                                        <button
                                                            ...
                                                            className="p-2 rounded-lg bg-primary/10 border border-primary/20 text-primary hover:bg-primary hover:text-foreground transition-all active:scale-90"
                                                        ...
                                                        <button
                                                            ...
                                                            className="p-2 rounded-lg bg-blue-500/10 border border-blue-500/20 text-blue-400 hover:bg-blue-500 hover:text-foreground transition-all active:scale-90"
                                                            title="Modifier"
                                                        >
                                                        ...
                                                        <button
                                                            ...
                                                            className="p-2 rounded-lg bg-red-500/10 border border-red-500/20 text-red-400 hover:bg-red-500 hover:text-foreground transition-all active:scale-90"
                                                            title="Supprimer"
                                                        >
                                            ...
                                            return (
                                                <td key={col.key} className="px-6 py-4 text-muted whitespace-nowrap border-r border-border last:border-r-0" style={{ minWidth: col.width }}>
                                                    <div className="truncate font-medium group-hover:text-primary transition-colors" style={{ minWidth: col.width }}>
                                                        {val !== "" && val !== null ? String(val) : <span className="text-slate-300 italic text-xs">Non renseigné</span>}
                                                    </div>
                                                </td>
                                            );
                                        })}
                ...
                {!initialLoading && filteredRows.length > 0 && (
                    <div className="px-8 py-4 border-t border-white/5 flex flex-col sm:flex-row items-center justify-between gap-4 bg-white/[0.01]">
                        <div className="flex items-center gap-3">
                            <span className="text-xs text-muted font-bold uppercase tracking-widest">Afficher</span>
                        ...
                        <div className="flex items-center gap-6">
                            <span className="text-[10px] font-black text-slate-600 uppercase tracking-widest">
                                Page <span className="text-foreground">{currentPage}</span> sur {totalPages || 1}
                            </span>
                            <div className="flex items-center gap-2">
                                <button
                                    onClick={() => setCurrentPage(p => Math.max(1, p - 1))}
                                    disabled={currentPage === 1}
                                    className="p-2.5 rounded-xl bg-white/5 border border-white/10 text-foreground disabled:opacity-20 disabled:cursor-not-allowed hover:bg-white/10 transition-all active:scale-95 group"
...
function EditModal({ row: initialRow, references, onClose, onSave }: { row: any, references: any, onClose: () => void, onSave: (id: number, data: any) => void }) {
    ...
    if (isLoading || !references) return (
        <div className="reporting-modal-wrapper">
            <div className="reporting-modal-overlay" onClick={onClose} />
            <div className="reporting-modal-container flex flex-col items-center justify-center min-h-[400px]">
                <svg className="animate-spin text-primary w-12 h-12 mb-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 12a9 9 0 1 1-6.219-8.56" /></svg>
                <p className="text-muted font-bold text-xs uppercase tracking-widest">Chargement du formulaire...</p>
            </div>
        </div>
    );

    const inputCls = "w-full px-6 py-4 rounded-2xl bg-white border border-border text-foreground focus:border-primary/50 transition-all outline-none";
    const labelCls = "text-[10px] font-bold text-muted uppercase ml-1";
    ...
    return (
        <div className="reporting-modal-wrapper">
            ...
            {/* Container */}
            <div className="reporting-modal-container animate-in fade-in zoom-in duration-300">

                {/* Header */}
                <div className="reporting-modal-header">
                    <div className="pl-20">
                        <h2 className="text-2xl font-black text-foreground uppercase tracking-tight">Modification Fiche</h2>
                        <p className="text-xs text-muted font-bold uppercase tracking-widest mt-1">ID: {formData.excel_id || formData.id} — {formData.raison_sociale || "—"}</p>
                    </div>
                    <button onClick={onClose} className="p-3 rounded-2xl bg-white/5 border border-border text-foreground hover:bg-red-500/10 hover:text-red-500 transition-all">
                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><path d="M18 6 6 18M6 6l12 12" /></svg>
                    </button>
                </div>

                {/* Form Body - Scrollable */}
                <div className="reporting-modal-body" style={{ maxHeight: "calc(90vh - 200px)" }}>
                    ...
                    <div className="space-y-6 pt-10 border-t border-white/5">
                        ...
                        <div className="space-y-2">
                            ...
                            <textarea className="w-full px-6 py-4 rounded-2xl bg-white/[0.03] border border-white/10 text-foreground focus:border-primary/50 transition-all outline-none min-h-[100px]" value={formData.description || ""} onChange={(e) => updateField('description', e.target.value)} />
                        </div>
                    </div>
                    ...
                    <div className="space-y-6 pt-10 border-t border-white/5">
                        ...
                        <div className="space-y-2">
                            ...
                            <textarea className="w-full px-6 py-4 rounded-2xl bg-white/[0.03] border border-white/10 text-foreground focus:border-primary/50 transition-all outline-none min-h-[80px]" value={formData.actions || ""} onChange={(e) => updateField('actions', e.target.value)} />
                        </div>
                        <div className="space-y-2">
                            <label className={labelCls}>Observations Terrain</label>
                            <textarea className="w-full px-6 py-4 rounded-2xl bg-white/[0.03] border border-white/10 text-foreground focus:border-primary/50 transition-all outline-none min-h-[100px]" value={formData.observations || ""} onChange={(e) => updateField('observations', e.target.value)} />
                        </div>
                    </div>
                </div>
                ...
                <div className="px-10 py-6 border-t border-white/5 bg-white/[0.01] flex justify-between items-center shrink-0">
                    <button onClick={onClose} className="px-8 py-4 rounded-2xl text-muted font-bold uppercase text-[10px] tracking-widest hover:text-foreground transition-all">
                        Annuler
                    </button>
                    <button onClick={handleSave} disabled={isSaving} className="flex items-center gap-3 px-10 py-4 rounded-2xl bg-primary hover:bg-primary-hover text-foreground text-[10px] font-black uppercase tracking-widest shadow-xl shadow-primary/20 transition-all active:scale-95 disabled:opacity-50">
