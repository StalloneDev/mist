...
export async function getReportingRowById(id: number) {
    try {
        const data = await db.reportingVisite.findUnique({
            where: { id }
        });
        
        if (!data) return { success: false, error: "Introuvable" };
...
export async function updateReportingRow(id: number, data: any) {
    try {
        ...
        const { ville, ...reportingUpdateData } = updateData;

        const result = await db.$transaction(async (tx) => {
            // 1. Update Reporting Record
            const updatedRow = await tx.reportingVisite.update({
                where: { id: rowId },
                data: reportingUpdateData
            });

            // 2. Bidirectional Sync: Update Enterprise if relevant fields changed
            if (updatedRow.entreprise_id && (reportingUpdateData.raison_sociale || ville)) {
                await tx.entreprise.update({
                    where: { id: updatedRow.entreprise_id },
                    data: {
                        ...(reportingUpdateData.raison_sociale && { raison_sociale: reportingUpdateData.raison_sociale }),
                        ...(ville && { ville: ville }),
                    }
                });
            }
