...
export default function EntrepriseDetailClient({ entreprise: initialEntreprise, secteurs }: EntrepriseDetailClientProps) {
    ...
    return (
        <main className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-8 sm:space-y-10">

            {/* Header Section */}
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-8 border-b border-border">
                <div className="flex items-start gap-6">
                    <Link href="/entreprises" className="mt-1 w-12 h-12 rounded-2xl bg-surface-2 hover:bg-surface-3 border border-border flex items-center justify-center text-muted hover:text-foreground transition-all">
                    ...
                    <div className="space-y-2">
                        <div className="flex flex-wrap items-center gap-3">
                            <h1 className="text-2xl sm:text-3xl lg:text-4xl font-black text-foreground tracking-tight">{entreprise.raison_sociale}</h1>
                ...
                <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
                    <button 
                        ...
                        className="flex-1 sm:flex-none px-6 py-3 rounded-2xl bg-surface-2 hover:bg-surface-3 border border-border text-sm font-black text-foreground transition-all"
                    >
                        Modifier
                    </button>
                    <Link href="/visites/nouvelle" className="flex-1 sm:flex-none flex justify-center items-center px-6 py-3 rounded-2xl bg-primary hover:bg-primary/90 text-sm font-black text-white transition-all shadow-lg shadow-primary/20">
                        + Nouvelle Visite
                    </Link>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                ...
                <div className="space-y-6 sm:space-y-10">
                    <section className="premium-panel p-4 sm:p-6 lg:p-8">
                        ...
                        <div className="space-y-4">
                            {entreprise.contacts.map((c: any) => (
                                <div key={c.id} className="p-5 rounded-2xl bg-surface-2 border border-border hover:border-primary/30 transition-all group relative">
                                    <button 
                                        onClick={() => {
                                            setSelectedContact(c);
                                            setIsEditContactOpen(true);
                                        }}
                                        className="absolute top-4 right-4 p-2 opacity-0 group-hover:opacity-100 bg-surface-3 hover:bg-primary/10 rounded-xl text-muted hover:text-primary transition-all"
                                    >
                                    ...
                                    <div className="font-black text-foreground mb-0.5 group-hover:text-primary transition-colors">{c.nom}</div>
                                    <div className="text-[10px] text-muted font-black uppercase tracking-wider mb-4">{c.fonction}</div>
                                    <div className="space-y-2">
                                        {c.email && (
                                            <div className="flex items-center gap-2 text-xs text-muted font-medium">
                                                <svg className="text-muted/40" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><rect width="20" height="16" x="2" y="4" rx="2" /><path d="m22 7-8.53 5.33a2 2 0 0 1-2.14 0L2 7" /></svg>
                                                <span className="truncate">{c.email}</span>
                                            </div>
                                        )}
                                        {c.telephone && (
                                            <div className="flex items-center gap-2 text-xs text-muted font-medium">
                                                <svg className="text-muted/40" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l2.28-2.28a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z" /></svg>
                            ...
                            {entreprise.contacts.length === 0 && (
                                <div className="text-center py-8 rounded-2xl border border-dashed border-border text-muted text-xs font-bold uppercase tracking-widest bg-surface-2">
                                    Aucun contact
                                </div>
                            )}
                        </div>
                    ...
                    <section className="premium-panel p-4 sm:p-6 lg:p-8">
                        ...
                        <div className="space-y-6">
                            <div>
                                <div className="text-[10px] font-black uppercase tracking-widest text-muted mb-2">Adresse Principale</div>
                                <p className="text-sm font-medium text-foreground leading-relaxed">{entreprise.adresse || "Non renseignée"}</p>
                            </div>
                            <div>
                                <div className="text-[10px] font-black uppercase tracking-widest text-muted mb-2">Membre depuis</div>
                                <p className="text-sm font-black text-foreground">{new Date(entreprise.date_creation).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' })}</p>
                            </div>
                        </div>
                    </section>
                </div>
                    ...
                    <section className="premium-panel p-4 sm:p-6 lg:p-8">
                        ...
                        <div className="relative space-y-12 before:absolute before:left-[11px] before:top-2 before:bottom-2 before:w-0.5 before:bg-white/5">
                                ...
                                <div key={v.id} className="relative pl-10">
                                    ...
                                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
                                        ...
                                        <div className="flex -space-x-2">
                                            <div className="w-8 h-8 rounded-full bg-primary/20 border-2 border-surface-1 flex items-center justify-center text-[10px] font-black text-primary">
                                    ...
                                    {v.observations && (
                                        <div className="p-4 rounded-2xl bg-surface-2 border border-border text-sm text-muted italic mb-6 leading-relaxed">
                                            "{v.observations}"
                                        </div>
                                    )}
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        {v.projets.map((p: any) => (
                                            <div key={p.id} className="p-5 rounded-2xl bg-surface-2 border border-border hover:border-primary/20 transition-all shadow-sm">
                                                <div className="flex justify-between items-start mb-4">
                                                    ...
                                                    <span className="text-[10px] text-muted font-bold">PROJET #{p.id}</span>
                                                </div>
                                                <p className="text-sm font-bold text-foreground line-clamp-2 mb-4 leading-relaxed">{p.description}</p>
                            ...
                            {entreprise.visites.length === 0 && (
                                <div className="text-center py-16 rounded-3xl bg-surface-2 border border-dashed border-border">
                                    <p className="text-muted text-xs font-black uppercase tracking-[0.2em]">Historique vierge</p>
                                </div>
                            )}
                        </div>
                    </section>
